...
def armazenar(telefone, endereco, nascimento, genero):
    erro = False
    try:
        banco = sqlite3.connect("data_prof_cadastro.db")
        cursor = banco.cursor()

        cursor.execute("CREATE TABLE IF NOT EXISTS data_prof_cadastro(telefone, endereco, nascimento, genero)")
        cursor.execute("INSERT INTO data_prof_cadastro VALUES (?, ?, ?, ?)", (telefone, endereco, nascimento, genero))
        banco.commit()
        banco.close()
